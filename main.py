...
class main (QMainWindow):
    ...
    def readfileButtonclicked(self):
        ...
        print(self.buttontab.minimumHeight())
        ...
        if path=="":
            ...
            self.Table6.setHorizontalHeaderLabels(["文件名","Project","Name","part","Operator","Desc","Excited_Peroid(ms)","Excited_Time(ms)","Acq_Delay_Time(us)","Gate_Time(ms)","Count_Num_per_gate","Repeat_Times","Acq_Gate_Times","Interval_per_Gate(us)","Channel_Number"])
    ...
    def updataTable45(self):
        ...
        if(self.Table2V!=""):
            ...
            if (self.data.filelist[self.Table2V].paras["双曲线拟合"] != []):
                ...
                self.Table4.setHorizontalHeaderLabels(["文件名","前截点","后截点","Max","I_0","τ","Γ","D","τ/Γ","R_square","SSE","MSE","RMSE"])
                p = 0
            ...
            if (self.data.filelist[self.Table2V].paras["指数拟合"] != []):
                ...
                self.Table5.setHorizontalHeaderLabels(["文件名","前截点","后截点","Max","I_0","τ","D","R_square","SSE","MSE","RMSE"])
                p = 0
    ...
    def Table4VerticalHeaderClick(self,index):
        ...
        title=self.Table2V+"文件前截"+str(self.data.filelist[self.Table2V].paras["双曲线拟合"][index].cutstartnumspot1)+"个点-后截"+str(self.data.filelist[self.Table2V].paras["双曲线拟合"][index].cutendnumspot1)+"个点的双曲线拟合图"
        self.figure.fig.canvas.draw_idle()
        self.figure.axes.clear()

        if (self.showcut == 0):
            self.figure.axes.plot(self.data.filelist[self.Table2V].Pro_Data1_X[0:numstart1 + 1],
                                  self.data.filelist[self.Table2V].Pro_Data1[0:numstart1 + 1], "--", color="green")  # 前半段
            ...
            self.figure.axes.scatter(self.data.filelist[self.Table2V].Pro_Data1_X[numend1:],
                                     self.data.filelist[self.Table2V].Pro_Data1[numend1:], color="green", alpha=0.3)  # 后半段段
        
        
        
        # 原始曲线

        self.figure.axes.plot(self.data.filelist[self.Table2V].Pro_Data1_X[numstart1:numend1+1],self.data.filelist[self.Table2V].Pro_Data1[numstart1:numend1+1],color="blue") #中段

        # 原始散点
        self.figure.axes.scatter(self.data.filelist[self.Table2V].Pro_Data1_X[numstart1:numend1+1], self.data.filelist[self.Table2V].Pro_Data1[numstart1:numend1+1],color="blue",alpha=0.3)
    ...
    def Table5VerticalHeaderClick(self, index):
            ...
            title = self.Table2V + "文件前截" + str(self.data.filelist[self.Table2V].paras["指数拟合"][
                index].cutstartnumspot1) + "个点-后截" + str(self.data.filelist[self.Table2V].paras["指数拟合"][
    ...
    def fitButtonlicked(self):
        ...
        if(self.cb1.isChecked()):
            self.data.Fitting(1,self.cb1ComboBoxstart.currentText(),startnum,endnum,title,self.b1low,self.b1top,self.b2low,self.b2top)
        if(self.cb2.isChecked()):
            self.data.Fitting(2,self.cb2ComboBoxstart.currentText(),startnum,endnum,title,self.b1low,self.b1top,self.b2low,self.b2top)
        if (self.cb3.isChecked()):
            self.data.Fitting(3, self.cb3ComboBoxstart.currentText(),startnum,endnum,title,self.b1low,self.b1top,self.b2low,self.b2top)
        if (self.cb4.isChecked()):
            self.data.Fitting(4, self.cb4ComboBoxstart.currentText(),startnum,endnum,title,self.b1low,self.b1top,self.b2low,self.b2top)
        ...
        title2=["文件名","ACQ_Time","Max(原始)","数据量","双曲Max","双曲前截","双曲后截","(双曲)R_square","(双曲)I_0", "(双曲)τ", "(双曲)Γ", "(双曲)D","(双曲)τ/Γ","指数Max","指数前截","指数后截","(指数)R_square","(指数)I_0", "(指数)τ", "(指数)D"]
        ...
        for key,value in self.data.filelist.items():
            ...
            #     for t in range(4):
            #         row.append(value.paras["双曲线积分拟合"][-1].para[t])
            #     else:
            #         row.append("")
            #         row.append("")
            #         row.append("")
            ...
            #     for t in range(3):
            #         row.append(value.paras["指数积分拟合"][-1].para[t])
            #     else:
            #         row.append("")
            #         row.append("")
            #         row.append("")
            self.Table2.setItem(i, 0, QTableWidgetItem(key))
            # self.Table2.setItem(i, 1, QTableWidgetItem(str(value.paras.Max)))
