...
class dataread():
    ...
    def readfiles(self,filepath):  #存入数据
        ...
        for f in self.filenames:
            self.statusBar().showMessage("正在转换 " + str(p) + "/" + str(len(self.filenames))+" "+f)
            data=dataclass()
    ...
    def writeXls(self,outpath):
        ...
        for key,value in self.filelist.items():
            ...
            if (value.paras["指数拟合"] != []):
                ...
                for spot in value.Pro_Data1[value.paras["指数拟合"][-1].cutstartnum1:value.paras["指数拟合"][-1].cutendnum1 + 1]:
                    ...
                    z += 1
            self.progressBar.setValue(i/len(self.filelist)*80)
            i += 1
        print("round2")
        # 文件信息
        inf.write(0, 0, "文件名")
        ...
        for key, value in self.filelist.items():
            ...
            self.progressBar.setValue(i / len(self.filelist) * 20+80)
            i+=1
        workbook.close()
        # 生成子矩阵文件夹
    ...
    def writesondataXls(self,outpath):
        ...
        p=1
        for filename, file in self.filelist.items():
            self.statusBar().showMessage("正在保存数据子矩阵 "+str(p)+"/"+str(len(self.filelist))+" "+os.path.splitext(filename)[0])
    ...
    def writesinglefiledata(self,outpath):
        ...
        errlist=[]
        for filename, file in self.filelist.items():
            self.statusBar().showMessage("正在保存单文件裁剪数据 " + str(p) + "/" + str(len(self.filelist)) + " " + os.path.splitext(filename)[0])
            self.progressBar.setValue(p / len(self.filelist) * 100)
            # try:
            ...
            p += 1
        if (exist == False):
            self.statusBar().showMessage("首次保存，已将单数据裁剪文件保存到数据目录下的‘单文件裁剪后拟合数据’！")
        else:
            if errlist==[]:
                self.statusBar().showMessage("单文件裁剪后拟合数据更新成功！")
            else:
                name=""
                for temp in errlist:
                    name+=temp
                self.statusBar().showMessage(name+"保存失败(文件被占用)，其余文件保存成功")
        self.progressBar.setVisible(False)
    ...
    def Fitting(self,funtype,method,startnum,endnum,filename,b1low,b1top,b2low,b2top):
        ...
        if(filename==""):
            ...
            for key,value in self.filelist.items():
                ...
                if(funtype==1):
                    self.statusBar().showMessage("正在进行双曲线拟合 " + str(p) + "/" + str(len(self.filenames))+" "+key)
                    try:
                        # print("双曲")
                        # print(b1low[0]*Max,b1low[1],b1low[2],b1low[3])
                        # print(b1top[0]*Max,b1top[1],b1top[2],b1top[3])
                        popt, pcov = curve_fit(fun1, x, y,maxfev=500000000,bounds=([b1low[0]*Max,b1low[1],b1low[2],b1low[3]],[b1top[0]*Max,b1top[1],b1top[2],b1top[3]]),method=method)
                    ...
                    if (value.paras["双曲线拟合"]!=[]):
                        for i in range(len(value.paras["双曲线拟合"])):
                            if ((value.paras["双曲线拟合"][i].cutstartnum1==starttruenum)and(value.paras["双曲线拟合"][i].cutendnum1==endtruenum)):
                    ...
                    # print(key)
                    self.statusBar().showMessage("正在进行指数拟合 " + str(p) + "/" + str(len(self.filenames))+" "+key)
                    #
                    # print(x)
                    # print(y)
                    try:
                        ...
                        popt, pcov = curve_fit(fun2, x, y,maxfev=50000000,bounds=([b2low[0]*Max,b2low[1],b2low[2]],[b2top[0]*Max,b2top[1],b2top[2]]),method=method)
                    ...
                    if (value.paras["双曲线拟合"] != []):
                        for i in range(len(value.paras["指数拟合"])):
                            if ((value.paras["指数拟合"][i].cutstartnum1 == starttruenum) and ( value.paras["指数拟合"][i].cutendnum1 == endtruenum)):
            ...
            try:
                if (funtype == 1):
                    ...
                    # print(b1low[0]*Max,b1low[1],b1low[2],b1low[3])
                    # print(b1top[0]*Max,b1top[1],b1top[2],b1top[3])
                    popt, pcov = curve_fit(fun1, x, y, maxfev=500000, bounds=([b1low[0]*Max,b1low[1],b1low[2],b1low[3]],[b1top[0]*Max,b1top[1],b1top[2],b1top[3]]))
                    yfit = fun1(xfit, popt[0], popt[1], popt[2], popt[3])
                    ytempfit = fun1(x, popt[0], popt[1], popt[2], popt[3])
                    R2 = getIndexes(ytempfit, y)
                    isexist = 0
                    ...
                    # print(b2low[0] * Max, b2low[1], b2low[2])
                    # print(b2top[0] * Max, b2top[1], b2top[2])
                    popt, pcov = curve_fit(fun2, x, y, maxfev=500000, bounds=([b2low[0]*Max,b2low[1],b2low[2]],[b2top[0]*Max,b2top[1],b2top[2]]))
                    yfit = fun2(xfit, popt[0], popt[1], popt[2])
                    ytempfit = fun2(x, popt[0], popt[1], popt[2])
                    R2=getIndexes(ytempfit, y)
                    isexist = 0
